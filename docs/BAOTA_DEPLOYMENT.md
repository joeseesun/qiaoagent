...
不要在宝塔安全或云安全组中开放 `3355`。默认 Compose 只把它绑定到 `127.0.0.1`，公网流量必须经过反向代理。
...
```yaml
ports:
  - "127.0.0.1:3356:3355"  # 改端口后仍只绑定 loopback
```

同时把宝塔反向代理目标改为 `http://127.0.0.1:3356`。
...
1. 在服务器执行：`curl -I http://127.0.0.1:3355`
2. 检查宝塔反向代理、域名解析和 HTTPS 配置
